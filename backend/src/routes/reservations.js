...
router.post('/',
  ...
  async (req, res) => {
    ...
    try {
      ...
      const dateTime = new Date(`${dateReservation}T${heureReservation}:00`);
      ...
      const personnesDejaReservees = reservationsExistantes._sum.nombrePersonnes || 0;
      const capaciteMax = 50; // À configurer selon les besoins
      
      if (personnesDejaReservees + nombrePersonnes > capaciteMax) {
        return res.status(400).json({
